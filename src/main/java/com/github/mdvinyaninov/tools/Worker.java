...
public class Worker implements Callable<Integer> {
    ...
    // datetime parameters
    Worker(RootConfiguration root_config, PanelConfig panel_config, String prefix, LocalDateTime from, LocalDateTime to,
    ...
    Worker(RootConfiguration root_config, PanelConfig panel_config, String prefix, Long from, Long to,
           CountDownLatch latch) {
        _init(root_config, panel_config, prefix, from.toString(), to.toString(), latch);
    }

    // relative parameters
    Worker(RootConfiguration root_config, PanelConfig panel_config, String prefix, String from, String to,
           CountDownLatch latch) {
        _init(root_config, panel_config, prefix, from, to, latch);
    }

    // String parameters
    private void _init(RootConfiguration root_config, PanelConfig panel_config, String prefix, String from, String to,
                       CountDownLatch latch){
        this.latch = latch;
        ...
        try {
            List<PanelVariable> vars = new ArrayList<>();
            if (root_config.getGlobalVariables() != null)
                vars.addAll(root_config.getGlobalVariables());
            if (panel_config.getPanelVariables() != null)
                vars.addAll(panel_config.getPanelVariables());

            List<Param> params = new ArrayList<>();
            if (root_config.getGlobalParams() != null)
                params.addAll(root_config.getGlobalParams());
            if (panel_config.getPanelParams() != null)
                params.addAll(panel_config.getPanelParams());
